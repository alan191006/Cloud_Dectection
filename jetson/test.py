...
MODEL_PATH = "./model_engine.trt"
RED_PATH   = "./95Cloud/95red/"
GREEN_PATH = "./95Cloud/95green/"
BLUE_PATH  = "./95Cloud/95blue/"
NIR_PATH   = "./95Cloud/95nir/"
GT_PATH    = "./95Cloud/95gt/"
...
class CloudDataset():
    ...
    def __getitem__(self, idx):
        x = np.expand_dims(np.ascontiguousarray(self.open_as_array(idx, invert=True), dtype=np.float32), 0)
...
def get_accuracy(dataset, model):
    ...
    for i in range(len(dataset)):
        ...
        predictions = torch.squeeze(torch.from_numpy(np.squeeze(model.predict(inp)[:, 1, :, :])))
...
for i in range(len(full_cloud_data)):
    ...
    out = cv2.inRange(np.squeeze(x).transpose(1, 2, 0), thickness_threshold, (1, 1, 1, 1))
...
thick_acc, thick_rec, thick_pre, thick_f1 = get_accuracy(thick_cloud_data, model)
...
np.savetxt("/home/lux/Desktop/Cloud_detection/import.csv", s, delimiter=",", fmt='%s')
